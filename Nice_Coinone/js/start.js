/**
 * Created by hy1 on 2017/9/11.
 */
var imgWidth=document.querySelector('#banner .logo').offsetWidth;
$('.infoList').css('width',imgWidth);
    function change(){
...
//清除定时器时候的重置定时器--封装
    ...
    function changeTo(num){
        $(".imgList").find("li").removeClass("active").eq(num).addClass('active');
    ...
    xAxis: [{
        ...
        axisLabel: {
            formatter: function (value) {
                return echarts.format.formatTime('                                                                         ', value);
            }
        }
    }, {
        type: 'category',
...
function checkPwd(){
    var self=this.value;
    var regPassword1=/[a-zA-Z]{1,}/,regPassword2=/[0-9]{1,}/;
    var result=regPassword1.test(self)&&regPassword2.test(self);
    if(self.length>=6&&result){
        //验证通过
        $(this).parent().siblings('.regtxt').css('display','none');
    }else{
        //验证没有通过
        $(this).parent().siblings('.regtxt').css('display','block');
    }
}
//密码验证
$('#myPassword').focusout(checkPwd);
$('#newPwd').focusout(checkPwd)
//手机验证
...
$('#remember').on('change',function(){
    $(this).siblings('.radio-img').toggleClass('check');
});
$("")
